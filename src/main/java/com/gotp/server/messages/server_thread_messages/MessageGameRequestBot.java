...
public class MessageGameRequestBot implements Message, Serializable, MessageWithSocket {
    ...
    public MessageType getType() {
        return MessageType.GAME_REQUEST_BOT;
    }
     ...
     * @param boardSize
     */
    public MessageGameRequestBot(final int boardSize) {
        if (boardSize < 1) {
     ...
     */
    public MessageGameRequestBot(final Socket requestingClient, final int boardSize) {
        if (boardSize < 1) {
            throw new IllegalArgumentException("Game size must be at least 1.");
        }
        this.boardSize = boardSize;
    ...
    public Message addClientSocket(final Socket socket) {
        return new MessageGameRequestBot(socket, this.boardSize);
    }
}
